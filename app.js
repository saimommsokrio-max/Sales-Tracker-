...
function loadState() {
  ...
  // 3. Fallback to embedded default state
  if (!loadedState) {
    loadedState = JSON.parse(JSON.stringify(EMBEDDED_DEFAULT_STATE));
  }

  // Ensure essential collections exist
  if (!loadedState.plans) loadedState.plans = JSON.parse(JSON.stringify(EMBEDDED_DEFAULT_STATE.plans || {}));
  if (!loadedState.plans['2026-9'] && EMBEDDED_DEFAULT_STATE.plans && EMBEDDED_DEFAULT_STATE.plans['2026-9']) {
    loadedState.plans['2026-9'] = JSON.parse(JSON.stringify(EMBEDDED_DEFAULT_STATE.plans['2026-9']));
  }
  if (!loadedState.companies || loadedState.companies.length === 0) {
    loadedState.companies = JSON.parse(JSON.stringify(EMBEDDED_DEFAULT_STATE.companies || DEFAULT_COMPANIES));
  }
  if (!loadedState.clientFollowups || loadedState.clientFollowups.length === 0) {
    loadedState.clientFollowups = JSON.parse(JSON.stringify(EMBEDDED_DEFAULT_STATE.clientFollowups || DEFAULT_CLIENT_FOLLOWUPS));
  }
  if (!loadedState.callLogs || loadedState.callLogs.length === 0) {
    loadedState.callLogs = JSON.parse(JSON.stringify(EMBEDDED_DEFAULT_STATE.callLogs || DEFAULT_CALL_LOGS));
  }
